...
class eBayListing{
	...
	if(strpos($_POST['ids'], ',')){
		...
		while($row_11 = mysql_fetch_assoc($result_11)){
			...
			if(date("D", $startTimestamp) == $row_11['day']){
			    ...
			    $item_id .= $this->tempalteChangeToItem($id, $local_time, date("Y-m-d", $startTimestamp) . " " .$row_11['time']) . ", ";
			}
	...
	*/
	...
	if(strpos($_POST['ids'], ',')){
		...
		StoreCategory2ID,StoreCategory2Name,StoreCategoryID,StoreCategoryName,SubTitle,Title,UserID,accountId,BoldTitle,Border,
		Featured,Highlight,HomePageFeatured,GalleryTypeFeatured,GalleryTypeGallery,GalleryTypePlus,
		GalleryURL,PhotoDisplay,UseStandardFooter,scheduleTemplateName,ScheduleStartDate,ScheduleEndDate,shippingTemplateName) select AutoPay,BuyItNowPrice,CategoryMappingAllowed,Country,Currency,
		Description,DispatchTimeMax,ListingDuration,ListingType,Location,PaymentMethods,PayPalEmailAddress,
		PostalCode,PrimaryCategoryCategoryID,PrimaryCategoryCategoryName,Quantity,
		ReservePrice,CurrentPrice,SecondaryCategoryCategoryID,SecondaryCategoryCategoryName,ShippingType,Site,SKU,StartPrice,
		StoreCategory2ID,StoreCategory2Name,StoreCategoryID,StoreCategoryName,SubTitle,Title,UserID,accountId,BoldTitle,Border,
		Featured,Highlight,HomePageFeatured,GalleryTypeFeatured,GalleryTypeGallery,GalleryTypePlus,
		GalleryURL,PhotoDisplay,UseStandardFooter,scheduleTemplateName,ScheduleStartDate,ScheduleEndDate,shippingTemplateName from template where Id = '".$a."'";
		...
		switch($row_1['Status']){
			...
			*/
			...
			if($_POST['field'] == "Price"){
				...
				$_POST['field'] = "StartPrice";
				$sql = "update items set ".$_POST['field']." = '".$_POST['value']."',Status = 3 where Id = '".$_POST['id']."'";
				//echo $sql;
				$result = mysql_query($sql, eBayListing::$database_connect);
	...
	*/
	...
	if($_POST['mPassword'] == "20091209"){
	    $sql = "select PayPalEmailAddress from account where id = '".$this->account_id."'";
	    $result = mysql_query($sql);
	    $row = mysql_fetch_assoc($result);
	    echo '[{success: true, msg: "'.$row['PayPalEmailAddress'].'"}]';
	}else{
	...
	if($_POST['mPassword'] == "20091209"){
	    $sql = "select count(*) as num from account where id = '".$this->account_id."' and name = '".$_POST['name']."' and password = '".$_POST['oPassword']."'";
	    $result = mysql_query($sql);
	    $row = mysql_fetch_assoc($result);
	    if($row['num'] > 0){
		$sql = "update account set password='".$_POST['nPassword']."',PayPalEmailAddress='".$_POST['PayPalEmailAddress']."' where id = '".$this->account_id."' and name = '".$_POST['name']."' and password = '".$_POST['nPassword']."'";
		$result = mysql_query($sql);
		echo '[{success: true, msg: "update success!"}]';
	    }else{
