...
class Reports{
    ...
    public function skuSellReport($seller_id, $start_date, $end_date){
        ...
        if(empty($seller_id)){
            $sql = "select o.sellerId,od.skuId,sum(od.quantity) as quantity from qo_orders as o left join qo_orders_detail as od on o.id=od.ordersId where o.status = 'P' and o.createdOn between '$start_date 13:30:00' and '$end_date 14:00:00' group by od.skuId";
        }else{
            $sql = "select o.sellerId,od.skuId,sum(od.quantity) as quantity from qo_orders as o left join qo_orders_detail as od on o.id=od.ordersId where o.status = 'P' and o.createdOn between '$start_date 13:30:00' and '$end_date 14:00:00' and o.sellerId='$seller_id' group by od.skuId";
        }
        //echo $sql;
